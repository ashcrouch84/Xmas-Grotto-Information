...
namespace Xmas_Grotto_Information
{
    public partial class frmSettings : Form
    {
        ...
        private void loadSettings()
        {
            ...
            txtWait.Text = Properties.Settings.Default.giWait.ToString();
            txtLocal.Text = Properties.Settings.Default.giLocalSave.ToString();
            txtMonitor.Text = Properties.Settings.Default.giMonitorLoc.ToString();
            txtCam.Text = Properties.Settings.Default.giCamLoc.ToString();
            txtGrottoLoc.Text = Properties.Settings.Default.giGrottoLoc.ToString();
            txtMonitorInterval.Text = Properties.Settings.Default.giMonitorInt.ToString();
            txtQuestionFileLocation.Text = Properties.Settings.Default.giQuestionFile.ToString();
            txtCrypto.Text = Properties.Settings.Default.giDCPW.ToString();
        ...
        private void loadStatusList()
        {
            ...
            {
                lstStatus.Items.Add(strTemp[i].ToString());
            }
        }
        ...
        private void loadQuestions()
        {
            try
            {
                ...
                //check check boxes
                strChildChecked = Properties.Settings.Default.giChildChecked.ToString().Split(',');
                ...
                //check check boxes
                strAdultChecked = Properties.Settings.Default.giAdultChecked.ToString().Split(',');
                ...
                //check check boxes
                strFamilyChecked = Properties.Settings.Default.giFamilyChecked.ToString().Split(',');
            ...
            {
                MessageBox.Show("Can't find question file, please check location and reload settings?", "Missing file", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        {
            txtHost.Text = Properties.Settings.Default.giFTPHost.ToString();
            txtUsername.Text = Properties.Settings.Default.giFTPUsername.ToString();
            txtFTPPassword.Text = Properties.Settings.Default.giFTPPassword.ToString();
            txtPort.Text = Properties.Settings.Default.giFTPPort.ToString();
            txtRFAdult.Text = Properties.Settings.Default.giRFAdult.ToString();
            txtRFChild.Text = Properties.Settings.Default.giRFChild.ToString();
            txtRFFamily.Text = Properties.Settings.Default.giRFFamily.ToString();
        }
        ...
        private void cmdSaveFTP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Properties.Settings.Default.giFTPHost = txtHost.Text;
                Properties.Settings.Default.giFTPPassword = txtFTPPassword.Text;
                Properties.Settings.Default.giFTPUsername = txtUsername.Text;
                Properties.Settings.Default.giFTPPort = Int32.Parse(txtPort.Text);
        ...
        private void cmdSaveInterval_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    Properties.Settings.Default.giWait = Int32.Parse(txtWait.Text.Replace(",", " "));
                    Properties.Settings.Default.Save();
                }
                catch
                {
        ...
        private void cmdBrowseSave_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                Properties.Settings.Default.giLocalSave = folderBrowserDialog1.SelectedPath;
                Properties.Settings.Default.Save();
            }
        }
        ...
        private void cmdMonitor_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                Properties.Settings.Default.giMonitorLoc = folderBrowserDialog1.SelectedPath;
                Properties.Settings.Default.Save();
            }
        }
        ...
        private void cmdCam1_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                txtCam.Text = folderBrowserDialog1.SelectedPath;
                Properties.Settings.Default.giCamLoc = folderBrowserDialog1.SelectedPath;
                Properties.Settings.Default.Save();
            }
        }

        private void cmdQuestionFileLoc_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                txtQuestionFileLocation.Text = folderBrowserDialog1.SelectedPath;
                Properties.Settings.Default.giQuestionFile = folderBrowserDialog1.SelectedPath;
                Properties.Settings.Default.Save();
            }
        }
        ...
        private void updateFamilyChecked()
        {
            if (bLoaded == true)
            {
                ...
                Properties.Settings.Default.giFamilyChecked = famToSave;
                Properties.Settings.Default.Save();
            }
        }
        ...
        private void updateChildChecked()
        {
            if (bLoaded == true)
            {
                ...
                Properties.Settings.Default.giChildChecked = childToSave;
                Properties.Settings.Default.Save();
            }
        }
        ...
        {
            Properties.Settings.Default.giDCPW = txtCrypto.ToString();
            Properties.Settings.Default.Save();
        }
        ...
        private void updateAdultChecked()
        {
            if (bLoaded == true)
            {
                ...
                Properties.Settings.Default.giAdultChecked = adultToSave;
                Properties.Settings.Default.Save();
            }
        }

        private void cmdAddStatus_Click(object sender, EventArgs e)
        {
            if (txtStatus.Text != "")
            {
                lstStatus.Items.Add(txtStatus.Text);
                string strTemp = lstStatus.Items[0].ToString().Replace(@","," ");
                for (i=1;i<lstStatus.Items.Count;i++)
                {
                    strTemp = strTemp + "," + strTemp[i].ToString().Replace(@",", " ");
                }
                Properties.Settings.Default.giStatusList = strTemp;
                Properties.Settings.Default.Save();
            }
        }

        private void cmdDeleteStatus_Click(object sender, EventArgs e)
        {
            if (lstStatus.SelectedIndex  >-1)
            {
                lstStatus.Items.RemoveAt(lstStatus.SelectedIndex);
                string strTemp = "";
                if (lstStatus.Items.Count > 0)
                {
                    strTemp = lstStatus.Items[0].ToString().Replace(@",", " ");
                    for (i=1;i<lstStatus.Items.Count;i++)
                    {
                        strTemp = strTemp + "," + lstStatus.Items[i].ToString().Replace(@",", " ");
                    }
                }
                Properties.Settings.Default.giStatusList = strTemp;
                Properties.Settings.Default.Save();
        ...
        private void cmdGrottoLoc_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                txtGrottoLoc.Text = folderBrowserDialog1.SelectedPath;
                Properties.Settings.Default.giGrottoLoc = folderBrowserDialog1.SelectedPath;
                Properties.Settings.Default.Save();
            }
        }

        private void cmdMOnitorSave_Click(object sender, EventArgs e)
        {
            if (txtMonitorInterval.Text == "")
            { }
            else
            {
                try
                {
                    Properties.Settings.Default.giMonitorInt = Int32.Parse(txtMonitorInterval.Text.Replace(",", " "));
                    Properties.Settings.Default.Save();
                }
                catch
                {
